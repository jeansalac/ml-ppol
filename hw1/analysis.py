...
def main():
	...
	# print>>graf_time, graffiti['Type of Service Request'].groupby([graffiti['Creation Date'].dt.year]==2017, [graffiti['Creation Date'].dt.month]).agg({'count'})
 ...
 #    graf_resp = graffiti['Response Time'].groupby([graffiti['Response Time'], graffiti['ZIP Code']]).agg({'count'})
	...
	print>>build_time, buildings['SERVICE REQUEST TYPE'].groupby([buildings['DATE SERVICE REQUEST WAS RECEIVED'].dt.year]==2017, [buildings['DATE SERVICE REQUEST WAS RECEIVED'].dt.month]).agg({'count'})
	...
	print>>alleys_time, alleys['Type of Service Request'].groupby([alleys['Creation Date'].dt.year]==2017, [alleys['Creation Date'].dt.month]).agg({'count'})
